...
public class CommentService {
    ...
    public CommentResponse updateComment(Long commentId, String username, CommentRequest request) {
        ...
        // ✅ VÁ LỖI IDOR: Check xem user có quyền sửa comment này không
        String userRole = getUserRole();
        boolean isOwner = comment.getUser().getUsername().equals(username);
        boolean isAdmin = "ADMIN".equals(userRole);
        
        if (!isOwner && !isAdmin) {
            throw new RuntimeException("Lỗi IDOR: Bạn không có quyền sửa bình luận của người khác! (Chỉ chủ nhân hoặc admin mới được)");
        }
    ...
    public void deleteComment(Long commentId, String username) {
        ...
        // ✅ VÁ LỖI IDOR: Check xem user có quyền xóa comment này không
        String userRole = getUserRole();
        boolean isOwner = comment.getUser().getUsername().equals(username);
        boolean isAdmin = "ADMIN".equals(userRole);
        
        if (!isOwner && !isAdmin) {
            throw new RuntimeException("Lỗi IDOR: Bạn không có quyền xóa bình luận của người khác! (Chỉ chủ nhân hoặc admin mới được)");
        }
        
        commentRepository.delete(comment);
    }

    // ✅ Helper method: Lấy role của user hiện tại từ SecurityContext
